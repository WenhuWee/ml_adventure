...
def _main():
    floydOutPutPerfix = '/output/'
    floydInputPutPerfix = '/data/'
    # annotation_path = 'data/lebronjames_output_yolo/data/train.txt'
    annotation_path = '/data/train.txt'
    log_dir = floydOutPutPerfix + 'logs/000/'
    log_dir = floydOutPutPerfix
    with open(log_dir + 'test.txt', 'w') as f:  
        f.write('some stuff')  

    # classes_path = 'data/lebronjames_output_yolo/data/obj.names'
    ...
    model, bottleneck_model, last_layer_model = create_model(input_shape, anchors, num_classes,
            freeze_body=2, weights_path=floydInputPutPerfix + 'model/yolo.h5') # make sure you know what you freeze
    ...
    if True:
        ...
        bottlePath = floydOutPutPerfix + "bottlenecks.npz"
        if not os.path.isfile(bottlePath):
            ...
            np.savez(bottlePath, bot0=bottlenecks[0], bot1=bottlenecks[1], bot2=bottlenecks[2])
    
        # load bottleneck features from file
        dict_bot=np.load(bottlePath)
